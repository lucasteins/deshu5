...
def refresh_schema_caches():
    """数据变更后的 Schema 缓存刷新（免重启）：SchemaPreloader 从新库 information_schema
    重载（DB-first，非破坏）+ SchemaLoader/RAG 缓存失效。
    素材提资执行转换/复核确认、db_profile 档位切换后调用。失败仅告警。"""
    try:
...
def switch_database(profile: str) -> dict:
    ...
    # SchemaPreloader / SchemaLoader / RAG 缓存刷新
